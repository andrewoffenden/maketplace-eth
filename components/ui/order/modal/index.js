...
const OrderModal = ({product, onClose}) => {
  ...
  useEffect(() => {
    //whenever receive a new product useEffect executed and open a new modal
    if (!!product) {
      setIsOpen(true)
    }
  }, [product])
  ...
  return (
    <Modal isOpen={isOpen}>
      <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div className="sm:flex sm:items-start">
            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 className="mb-7 text-lg text-center bg-gray-400 p-4 font-bold leading-6 text-white rounded-lg" id="modal-title">
                {product.title}
              </h3>
        ...
        <div className="bg-gray-50 px-4 py-4 sm:px-8 sm:flex">
          <Button>
            Submit
          </Button>
          <Button
